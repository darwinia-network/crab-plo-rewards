...
const Page: React.FC = () => {
  ...
  const getUsersNftClaimed = async (after?: string): Promise<TypeGetUsersNFTClaimRemark> => {
  ...
  const handleClickCheckClaim = async () => {
    ...
    const remarks = await getUsersNftClaimed();
    while (remarks.pageInfo.hasNextPage) {
      const c = await getUsersNftClaimed(remarks.pageInfo.endCursor);
